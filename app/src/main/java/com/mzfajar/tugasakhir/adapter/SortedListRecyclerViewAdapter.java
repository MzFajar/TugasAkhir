...
abstract class SortedListRecyclerViewAdapter<Item, VH extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<VH> {

    private SortedList<Item> data = new SortedList<>(getItemClass(), new SortedList.Callback<Item>() {
        @Override
        public int compare(Item o1, Item o2) {
            return SortedListRecyclerViewAdapter.this.compare(o1, o2);
        }

        @Override
        public void onChanged(int position, int count) {
            SortedListRecyclerViewAdapter.this.onChanged(position, count);
        }

        @Override
        public boolean areContentsTheSame(Item oldItem, Item newItem) {
            return SortedListRecyclerViewAdapter.this.areContentsTheSame(oldItem, newItem);
        }

        @Override
        public boolean areItemsTheSame(Item item1, Item item2) {
            return SortedListRecyclerViewAdapter.this.areItemsTheSame(item1, item2);
        }

        @Override
        public void onInserted(int position, int count) {
            SortedListRecyclerViewAdapter.this.onInserted(position, count);
        }

        @Override
        public void onRemoved(int position, int count) {
            SortedListRecyclerViewAdapter.this.onRemoved(position, count);
        }

        @Override
        public void onMoved(int fromPosition, int toPosition) {
            SortedListRecyclerViewAdapter.this.onMoved(fromPosition, toPosition);
        }
    });
